...
const Card = ({ data, currentStyle, addToCart }) => {
  ...
  return (
    <div className={currentStyle}>
      <img className="imageDiv" src={data.imageUrl} alt="" />
      <h5>{data.title}</h5>
      <div className="cardInfo">
        <Link to={`/details/${data._id}`} id="detailsBtn">
          Details
        </Link>
        ...
        <p className="pCard">price: {data.price}$</p>
      </div>
    </div>
  );
};
