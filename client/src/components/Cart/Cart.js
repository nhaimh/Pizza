    ...
    <>
      {totalCart !== 0 && modal ? (
        <div className="BuyModal">
          <div className="BuyModalContent">
            ...
            />
            <p>
              <b>You`ve paid: </b> {`${totalCart}`} $
            </p>
            <button id="buymodalCancel" onClick={() => onHandler()}>
              Return
            </button>
            <p id="txtbuy">
              You can see your orders{" "}
              <Link to="/orders" onClick={() => onHandler()}>
                here
              </Link>{" "}
            </p>
          </div>
      ...
      <div className="cartBackground">
        ...
        <div id="totalDiv">
          ...
          <p id="price">{totalCart.toFixed(2)} $</p>
          <button id="buy" onClick={() => setModal(true)}>
            BUY
          </button>
          <button id="cancel" onClick={() => props.deleteAllCartProducts()}>
            CLEAR
          </button>
        </div>
      </div>
