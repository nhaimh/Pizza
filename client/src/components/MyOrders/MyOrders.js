  ...
  if (isLoading) {
    return (
      <div className="loaderContent">
        ...
        <img
          src="https://codemyui.com/wp-content/uploads/2019/01/Rotating-Pizza-Slice-Preloader.gif"
          width={400}
        />
      </div>
    );
            ...
            <ul>
              {data.map((item, index) => (
                <li className="listOrders" key={index}>
                  <p onClick={() => props.getProductsByOrderId(item.orderId)}>
                  ...
                  <p
                    className={`${
                      item.status == "In Progress" ? "status" : "statusOK"
                    }`}
                    onClick={() => orderUpdateHandler(item)}
                  >
                    Status: {item.status}
                  </p>
                </li>
      ...
      <div className="myOrdersContent">
        ...
        <ul>
          {props.orderedProducts.map(function (item, index) {
            return (
              <li key={index} className="myProductsList">
                <h2>{item.title}</h2>
