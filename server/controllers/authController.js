...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (!/^[a-zA-Z0-9]{3,}/g.test(username)) {
      throw new Error(
        "Username should be at least 3 characters long and should consist only english letters and digits!"
      );
    }

    if (!/^[a-zA-Z0-9]{3,}/g.test(password)) {
      throw new Error(
        "Password must be at least 3 characters long and consist only latin letters and digits!"
      );
    }
